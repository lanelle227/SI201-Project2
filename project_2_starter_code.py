...
def load_listing_results(html_file) -> list[tuple]: 
    """
    Load file data from html_file and parse through it to find listing titles and listing ids.

    Args: 
        html_file (str): The path to the HTML file containing the search results

    Returns: 
        A list of tuples containing listing title and listing id
    """

    pass 

def get_listing_details(listing_id) -> tuple: 
    """
    Parse through listing_id html file using regex to find desired return values.

    Args:
        listing_id (str): The listing id of the Airbnb listing
    
    Returns:
        A tuple containing the following details of the listing:
        (policy number, host type, host name, room type, num_reviews, and price)
    """
    pass

def create_listing_database(html_file) -> list[tuple]: 
    """
    ...
    Returns:
        A list of tuples. Each tuple should contain the following details of each listing:
        (listing title, listing id, policy number, host type, host name, room type, num_reviews, and price)
    """
    pass
...
def output_csv(data, filename) -> None: 
    ...
    Args:
        ...
        filename (str): The name of the CSV file to be created and saved to (e,g. "airbnb_data_output.csv")
...
"""
Part 5: validate_policy_numbers(data) -> list[tuple]

    INSTRUCTIONS:
    define the above function that takes in a list of tuples called data (the same format as the one returned by create_listing_database).

    Args:
        data (list): A list of tuples.
    Returns:
        A list of tuples. Each tuple should contain the following details of each listing with an invalid
        (listing id, host name, policy number)
"""
...
class TestCases(unittest.TestCase):
    ...
    def test_validate_policy_numbers(self):
        ...
        # and check that the return value is a list

        # ----------------------- uncomment this block after writing validate_policy_numbers() -----------------------
